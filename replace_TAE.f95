...
Measure=1                ! 1=OTAE; 2=RSumZm2; 3=OTAE or RSumZm2
...
call calcTAE(TableSwitch,T,U,maxj,nooft,Nm,TAE) 

!For each constraint table, calculate Rjm [Ujm-Tjm]; min Rjm; MaxRjm
call CalcRjm(TableSwitch,T,U,R,Nm,maxj,nooft,TAE)
...
!!!  endif ! if measure=1/2/3
  ...
  if (rep .eq. 1) then  
    ...
    call RemoveHHfromTjm(TableSwitch,T,maxj,nooft,H,HES_H,I,HES_I,FirstInd,LastInd,hhno,TableType)
    !Having found 'best' household (maxhh), need to add to Tjm
    call AddHHtoTjm(TableSwitch,T,maxj,nooft,H,HES_H,I,HES_I,FirstInd,LastInd,maxhh,TableType)
...
if (Estimate_Fit_Flag==1) then
  write(13,'(a10,2x,f6.1,i9,i8,2i5,i6,f8.2,f10.2)', advance= 'no') trim(AreaCode),  &
